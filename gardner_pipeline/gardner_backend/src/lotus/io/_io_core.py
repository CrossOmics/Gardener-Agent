...
def standardize_load(
    ...
    # Standardization & Safety Fixes
    adata.obs_names = adata.obs_names.astype(str)
    adata.var_names = adata.var_names.astype(str)

    if make_unique:
        # Now it is safe to call make_unique because all names are guaranteed to be strings.
    ...
    if convert_to_sparse and adata.X is not None and not issparse(adata.X):
        # Calculate sparsity to decide if conversion is worth it
