...
class DimReductionService:
    ...
    def run_pca(self, request: RunPCARequest) -> PCAResultDTO:
        ...
        fig_var = plt.gcf()
        pca_variance_filename = generate_filename('pca_variance', 'png')
        variance_plot_path = self.storage.save_file(
            fig_var,
            get_dataset_relative(request.project_id, request.dataset_id, DIMRED_SUBSPACE, pca_variance_filename)
        )
        plt.close(fig_var)
        ...
        scatter_filename = generate_filename('pca_scatter', 'png')
        scatter_path = self.storage.save_file(
            fig_scatter,
            get_dataset_relative(request.project_id, request.dataset_id, DIMRED_SUBSPACE, scatter_filename)
        )
        plt.close(fig_scatter)

        # Persist Result
        # Save snapshot
        snapshot_id = generate_business_id('s_node_pca')
    ...
    def build_neighborhood_graph(self, request: RunNeighborsRequest) -> NeighborsResultDTO:
        ...
        # Persist Result
        snapshot_id = generate_business_id('s_node_graph')
        file_name = f"{snapshot_id}.h5ad"
        relative_key = get_dataset_relative(request.project_id, request.dataset_id, SNAPSHOTS_SUBSPACE, file_name)
